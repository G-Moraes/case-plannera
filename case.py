...
class BrasilFeriados(AbstractHolidayCalendar):
    rules = [
        Holiday('Independência do Brasil', year = 2019, month = 9, day = 7),
...
def gerarDados(returnValue):
    ...
    for i in meanIndexes:

        objetivo['Dropsize'][i] = 0
    ...
    if returnValue == True:
        ...
        #x = pd.concat([x, objetivo.drop(columns = 'Remessas')], ignore_index=True)

        y = dados['Remessas']

        #y = pd.concat([y, objetivo['Remessas']], ignore_index=True)
